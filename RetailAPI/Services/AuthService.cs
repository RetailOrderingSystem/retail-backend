...
namespace RetailAPI.Services
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<string> RegisterAsync(RegisterDto dto)
        {
            ...
            // ensure we catch/save any exception and log state immediately after
            try
            {
                await _userRepository.SaveChangesAsync();

                // verification.Id will be populated by EF Core after SaveChanges
                _logger.LogInformation("EmailVerification saved. Id={Id}, UserId={UserId}, TokenMasked={Masked}, ExpiresAt={Exp}",
                    verification.Id, verification.UserId, MaskToken(verification.Token), verification.ExpiresAt);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed saving EmailVerification for user {UserId}", user.UserId);
                throw; // rethrow so the error is visible during testing
            }
        ...
        public async Task<string> VerifyEmailAsync(string email, string token)
        {
            ...
            // Support URL-encoded tokens: decode and trim
        ...
        public async Task<string> LoginAsync(LoginDto dto)
        {
            ...
            // Log masked OTP for debugging (do not log full OTP in production)
